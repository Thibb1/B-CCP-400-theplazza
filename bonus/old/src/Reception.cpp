...
int Reception::GetNewKitchen() {
    ...
    Kitchen newKitchen(CookingTime, Cooks, RefillTime);
    return kitchens;
}

void Reception::StartManagers() {
    std::thread test(std::thread(ThreadManager, std::ref(this)));
    Thread killer((std::thread(ThreadManager, std::ref(this))));
    killer.Detach();
}

    [[noreturn]] void Reception::ThreadManager(Reception &reception) {
